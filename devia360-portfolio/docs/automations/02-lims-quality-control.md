# 02 — LIMS: Laboratory Information Management System
...
**Raw material intake:**
- Step 1: Log the material in an inventory Excel (40 seconds)
- Step 2: Record lab results in a separate Excel (30 seconds)
...
- **Total: ~3.5 to 4.5 minutes per sample, spread across 3 formats**
...
```
Raw Material Intake        In-Process (Fabrication)       Finished Product
      │                           │                            │
      ▼                           ▼                            ▼
┌─────────────┐           ┌─────────────┐              ┌─────────────┐
...
| Metric | Before | After |
|--------|--------|-------|
| Raw material QC report | ~190 seconds (3 separate steps + paper) | 30 seconds (single workflow) |
...
**Time saved on raw material processing alone:** ~160 seconds per sample. With dozens of samples per week, this compounds into hours of recovered analyst time monthly.
...
The domain is chemistry. The pattern is triage.
